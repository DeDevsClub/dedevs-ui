...
class UpstashRedis {
    ...
    async pipeline(operations: Array<{ command: string[]; key?: string }>): Promise<any[]> {
        // Upstash supports pipeline operations
        const commands = operations.map(op => op.command);
...
// Store registry in Upstash Redis with optimized batch operations
async function storeRegistryInUpstash(registry: DatabaseRegistry): Promise<void> {
    const redis = new UpstashRedis();

    try {
        console.log('🚀 Storing registry in Upstash Redis...');
        ...
        operations.push({
            command: ['SET', 'dedevs:registry:complete', JSON.stringify(registry), 'EX', '86400'] // 24 hours
        });
        ...
        operations.push({
            command: ['SET', 'dedevs:registry:free', JSON.stringify(freeRegistry), 'EX', '86400']
        });

        // Store individual components
        for (const component of [...registry.components.free, ...registry.components.pro]) {
            operations.push({
                command: ['SET', `dedevs:component:${component.name}`, JSON.stringify(component), 'EX', '86400']
            });
            ...
            operations.push({
                command: ['SET', `dedevs:component:meta:${component.name}`, JSON.stringify(metadata), 'EX', '86400']
            });
        }

        // Store component lists by tier
        operations.push({
            command: ['SET', 'dedevs:components:free', JSON.stringify(registry.components.free.map(c => c.name)), 'EX', '86400']
        });
        operations.push({
            command: ['SET', 'dedevs:components:pro', JSON.stringify(registry.components.pro.map(c => c.name)), 'EX', '86400']
        });

        // Execute all operations in batches (Upstash has limits on pipeline size)
        const batchSize = 50;
        for (let i = 0; i < operations.length; i += batchSize) {
            const batch = operations.slice(i, i + batchSize);
            await redis.pipeline(batch);
            console.log(`📦 Processed batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(operations.length / batchSize)}`);
        }

        console.log('✅ Registry stored in Upstash Redis successfully');
        ...
        console.error('❌ Failed to store registry in Upstash Redis:', error);
        throw error;
    }
}

// Generate and store registries in Upstash
async function generateUpstashRegistry(): Promise<void> {
    ...
    // Store in Upstash
    console.log('💾 Storing registry in Upstash...');
    await storeRegistryInUpstash(databaseRegistry);
    ...
    console.log(`✅ Generated Upstash registry with ${allItems.length} components`);
...
// Utility function to retrieve components from Upstash
export async function getComponentFromUpstash(componentName: string, userTier: 'free' | 'pro' = 'free'): Promise<RegistryItem | null> {
    const redis = new UpstashRedis();
    ...
    try {
        ...
        console.error('Failed to retrieve component from Upstash:', error);
        return null;
    }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
    generateUpstashRegistry().catch(console.error);
}

export { generateUpstashRegistry, storeRegistryInUpstash };
